...
#加上tcn + 不选残差 (ATTEN池化 32)

class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim, hidden_dim), #(考虑用EEGNet中的 LinearWithConstraint代替)
            nn.ReLU(), #考虑用新型激活函数 TeLU 代替
            nn.Dropout(dropout),
...
class Deformer(nn.Module):
    ...
    def __init__(self, *, num_chan, num_time, layers_transformer, hidden_graph, num_head, alpha, temporal_kernel, num_kernel=64,
        ...
        self.act = nn.ELU()
        self.pool = nn.MaxPool1d(32, stride=32)
    ...
    #在进入 Transformer 之前对原始 EEG 信号进行初步的时序和空间特征提取
        ...
        # x = self.to_patch_embedding(x) #(16,64,1*500)=(16,64,500)
        ...
        x_mamba = self.proj(x)
        x_mamba = self.mamba(x_mamba)  # (b, n, dim_head)

        '''高性能版'''
        out_cat = torch.cat((x_trans, x_mamba), dim=-1)  # (b, n, 2*dim_head)
        x = self.fuse_proj(out_cat)  # (b, n, dim_head)

        # LayerNorm + Dropout
        out_proj = self.fuse_ln(x)
        x = self.fuse_dropout(out_proj)
...
if __name__ == "__main__":
    data = torch.ones((16, 28, 800))
    emt = Deformer(num_chan=28, num_time=800, layers_transformer = 1, hidden_graph = 64, num_head = 16, alpha = 0.25, temporal_kernel=11, num_kernel=64,
